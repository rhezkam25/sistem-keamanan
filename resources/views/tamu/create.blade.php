<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-semibold text-xl text-gray-800">Daftarkan Tamu Baru</h2>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg shadow p-6">
                <form method="POST" action="{{ route('tamu.store') }}" class="space-y-5">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <x-text-input id="nomor_id" name="nomor_id" type="text" class="mt-1 block w-full" :value="old('nomor_id')" required />
                        ...
                        <div>
                            ...
                            <select id="pejabat_id" name="pejabat_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm" required>
                                ...
                                @foreach($pejabatList as $p)
                                    <option value="{{ $p->id }}" {{ old('pejabat_id') == $p->id ? 'selected' : '' }}>{{ $p->name }} ({{ $p->jabatan }})</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="jenis_kendaraan" name="jenis_kendaraan" type="text" placeholder="Motor, Mobil, dll" class="mt-1 block w-full" :value="old('jenis_kendaraan')" />
                            <x-input-error :messages="$errors->get('jenis_kendaraan')" class="mt-1" />
                        </div>
                        <div>
                            <x-input-label for="plat_kendaraan" value="Plat Kendaraan" />
                            <x-text-input id="plat_kendaraan" name="plat_kendaraan" type="text" placeholder="B 1234 ABC" class="mt-1 block w-full" :value="old('plat_kendaraan')" />
                    ...
                    <div>
                        ...
                        <textarea id="tujuan_kunjungan" name="tujuan_kunjungan" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm" required>{{ old('tujuan_kunjungan') }}</textarea>
                        <x-input-error :messages="$errors->get('tujuan_kunjungan')" class="mt-1" />
                    </div>

                    <div class="flex justify-end gap-3 pt-2">
                        <a href="{{ route('tamu.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 text-sm">Batal</a>
                        <x-primary-button>Daftarkan Tamu</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
